...
<template>
	<div class="applications_run_page" v-if="model.form_save.item != null">
		...
		<div class="applications_run_page__left">
				...
				<div class="component_crud__table">
					<table>
						...
						<tr class="component_crud__row"
							v-for="modificator_id, index in model.form_save.item.modificators"
							:key="modificator_id"
						>
							<td>{{ index + 1 }}</td>
			...
			<div class="applications_run_page__variables applications_run_page__block">
				...
				<div class="applications_run_page__variable"
					v-for="variable, index in model.form_save.item.variables" :key="variable.name"
				>
			...
			<Tabs @select="onTabSelect($event)">
				<Tab name="template" label="Template">
					...
					<CodeMirror
						...
						v-bind:value="model.form_save.item.template.content"
						v-bind:crud="{
							field:
							{
								component_params:{
...
export const ApplicationsEditPage =
{
	...
	methods:
	{
		...
		getAppName: function()
		{
			return (this.model.form_save.item != null) ? this.model.form_save.item.name : "";
		},
		getTemplateName: function()
		{
			return (this.model.form_save.item != null && this.model.form_save.item.template != null) ?
				this.model.form_save.item.template.name : "";
		},
		...
		getStatus: function()
		{
			if (this.model.form_save.item.status == 0) return "STOPPED";
			if (this.model.form_save.item.status == 1) return "LAUNCHED";
			if (this.model.form_save.item.status == 2) return "STARTS UP";
			return "";
		},
		...
		{
			return [];
			let application_modificators = this.model.form_save.item.modificators;
		...
		onDialogAddModificatorButtonClick(button_text)
		{
			if (button_text == "yes")
			{
				...
				let index = this.model.form_save.item.modificators.indexOf(select_add_modificator);
				if (index == -1 && select_add_modificator > 0)
				{
					this.model.form_save.item.modificators.push(select_add_modificator);
				}
			}
		...
		onDialogDeleteModificatorButtonClick(button_text)
		{
			if (button_text == "yes")
			{
				...
				let index = this.model.form_save.item.modificators.indexOf(modificator.id);
				if (index > -1)
				{
					this.model.form_save.item.modificators.splice(index, 1);
				}
			}
